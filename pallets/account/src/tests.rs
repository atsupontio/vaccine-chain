...
fn should_cause_error_for_register_role() {
	...
	ExtBuilder::default()
		...
		assert_noop!(TemplateModule::approve_vm(Origin::signed(CHARLIE), BOB), Error::<Test>::NotSysMan);

		// approve again after approved
		assert_noop!(TemplateModule::approve_vm(Origin::signed(ALICE), BOB), Error::<Test>::NotClaimed);
		// approve entity which does not claim
		assert_noop!(TemplateModule::approve_vm(Origin::signed(ALICE), CHARLIE), Error::<Test>::NotClaimed);
	});
}
...
fn should_cause_error_for_register_info_about_vaccine() {
	ExtBuilder::default()
		...
		assert_noop!(TemplateModule::register_vac_type(Origin::signed(DAVE)), Error::<Test>::NotSysMan);

		// sysman(1) register vaccine type
		assert_ok!(TemplateModule::register_vac_type(Origin::signed(ALICE)));

		// NOT manufacture register vaccine type
		assert_noop!(TemplateModule::register_vac_info(Origin::signed(DAVE), Some(1)), Error::<Test>::NotManufacture);

		// sysman register not exist vaccine type
		assert_noop!(TemplateModule::register_vac_info(Origin::signed(BOB), Some(2)), Error::<Test>::NotRegisteredVaccineType);

		//register vaciine info
		assert_ok!(TemplateModule::register_vac_info(Origin::signed(BOB), Some(1)));

		// vaccine has been already registered
		assert_noop!(TemplateModule::register_vac_info(Origin::signed(BOB), Some(1)), Error::<Test>::VaccineIsRegistered);

		// approve vaccine
		assert_ok!(TemplateModule::approve_vaccine(Origin::signed(CHARLIE), Some(1)));

		// not VAO register vaciine info
		assert_noop!(TemplateModule::approve_vaccine(Origin::signed(DAVE), Some(1)), Error::<Test>::NotOrganization);

		// VAO register not exist vaciine id
		assert_noop!(TemplateModule::approve_vaccine(Origin::signed(CHARLIE), Some(2)), Error::<Test>::NotRegisteredVaccine);
...
fn should_cause_error_for_transfer_vaccine() {
	ExtBuilder::default()
		...
		assert_noop!(TemplateModule::transfer_vaccine(Origin::signed(FRANK), DAVE, Some(1)), Error::<Test>::NotManufacture);

		// transfer to myself
		assert_noop!(TemplateModule::transfer_vaccine(Origin::signed(BOB), BOB, Some(1)), Error::<Test>::TransferByMyself);

		// Not owner transfer vaccine
		assert_noop!(TemplateModule::transfer_vaccine(Origin::signed(DAVE), FRANK, Some(1)), Error::<Test>::WrongVaccineOwner);

		// transfer not exist vaccine
		assert_noop!(TemplateModule::transfer_vaccine(Origin::signed(BOB), DAVE, Some(3)), Error::<Test>::NotRegisteredVaccine);
		...
		assert_noop!(TemplateModule::receive_vaccine(Origin::signed(DAVE), BOB, Some(3)), Error::<Test>::NotRegisteredVaccine);

		// not manufacture or distributer receive vaccine
		assert_noop!(TemplateModule::receive_vaccine(Origin::signed(FRANK), BOB, Some(1)), Error::<Test>::NotManufacture);

		// not specified receiver
		assert_noop!(TemplateModule::receive_vaccine(Origin::signed(EVE), FRANK, Some(1)), Error::<Test>::NotVaccineBuyer);

		// receive from not vaccine owner
		assert_noop!(TemplateModule::receive_vaccine(Origin::signed(DAVE), EVE, Some(1)), Error::<Test>::WrongVaccineOwner);

		// receive vaccine
		assert_ok!(TemplateModule::receive_vaccine(Origin::signed(DAVE), BOB, Some(1)));

		// receive twice
		assert_noop!(TemplateModule::receive_vaccine(Origin::signed(DAVE), BOB, Some(1)), Error::<Test>::VaccineAlreadyMine);
	});
}
...
fn should_cause_error_for_get_confirm_vaccine() {
	ExtBuilder::default()
		...
		assert_noop!(TemplateModule::transfer_get_vaccine_right(Origin::signed(FRANK), DAVE, Some(1)), Error::<Test>::NotManufacture);
		// only manufacture or distributer
		assert_noop!(TemplateModule::transfer_get_vaccine_right(Origin::signed(DAVE), FRANK, Some(3)), Error::<Test>::NotRegisteredVaccine);
		// transfer to myself
		assert_noop!(TemplateModule::transfer_get_vaccine_right(Origin::signed(DAVE), DAVE, Some(1)), Error::<Test>::TransferByMyself);
		// not vaccine owner
		assert_noop!(TemplateModule::transfer_get_vaccine_right(Origin::signed(EVE), FRANK, Some(1)), Error::<Test>::WrongVaccineOwner);
		// receive vaccine inoculation right
		assert_ok!(TemplateModule::transfer_get_vaccine_right(Origin::signed(DAVE), FRANK, Some(1)));

		// not send final transfer to me
		assert_noop!(TemplateModule::confirm_vaccine(Origin::signed(FRANK), Some(DAVE), Some(3)), Error::<Test>::NotRegisteredVaccine);
		// not vaccine user
		assert_noop!(TemplateModule::confirm_vaccine(Origin::signed(GEORGE), Some(DAVE), Some(1)), Error::<Test>::NotVaccineBuyer);
		// not vaccine previous owner
		assert_noop!(TemplateModule::confirm_vaccine(Origin::signed(FRANK), Some(EVE), Some(1)), Error::<Test>::WrongVaccineOwner);
		// not transfer vaccine but transfer_get_vaccine_right
		assert_noop!(TemplateModule::confirm_vaccine(Origin::signed(FRANK), Some(DAVE), Some(2)), Error::<Test>::NotSendFinalTransfer);

		// confirm use vaccine
		assert_ok!(TemplateModule::confirm_vaccine(Origin::signed(FRANK), Some(DAVE), Some(1)));

		// not send final transfer to me
		assert_noop!(TemplateModule::confirm_vaccine(Origin::signed(FRANK), Some(DAVE), Some(1)), Error::<Test>::VaccineAlreadyMine);
